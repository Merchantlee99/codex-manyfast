...
이 이미지는 저장소의 local host harness에서 실제 Chromium으로 캡처했습니다. 플러그인과 동일한 widget renderer와 MCP `structuredContent` 계약을 사용합니다. 별도 목업은 아니지만 production Codex host 화면을 캡처한 것이라고 주장하지는 않습니다.

렌더링 fixture에는 다음 상태가 들어 있습니다.

- 사용자가 확정한 결정 2개
- 구현 blocker 1개를 포함한 열린 질문 2개
- 출처가 있는 외부 근거 1개
- 미확인 agent inference 1개
- 구현 중 발견된 추가 기획 1개
- immutable revision과 content hash
...
2026-07-14 로컬 검증 결과는 **11/11 테스트 통과**, plugin validation 통과, clean 임시 Codex home 설치·목록 확인 성공, 실제 Chromium 캡처 직접 확인입니다. 자세한 명령과 결과는 [검증 기록](./docs/verification/LOCAL_TEST.md)에 있습니다.

## 현재 한계

- 사용자 확인 문장을 기록하지만 실제 사람이 확인했다는 cryptographic proof는 아닙니다.
- local host harness로 Apps SDK-compatible widget 계약을 검증했으며, 위 이미지는 production Codex UI 캡처가 아닙니다.
...
npm run capture      # README screenshot 재생성
```
